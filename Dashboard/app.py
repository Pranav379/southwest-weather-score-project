...
if st.session_state.page == 'landing':
    ...
    # Step 1: Select flights based on weatherScore ranges
def sample_flights_by_score(df):
    ...
    for r_name, (low, high, n) in ranges.items():
        # Filter rows in the score range
        df_range = df[(df['weatherScore'] > low) & (df['weatherScore'] <= high)]
        # Shuffle and sample up to n flights
